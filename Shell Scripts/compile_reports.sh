...
for FILE in `ls $REPORTS_FOLDER/*.rdf`; do
  OUTPUT=${FILE/rdf/rep}
  echo "$FILE -> $OUTPUT"
  $DOMAIN_HOME/reports/bin/rwconverter.sh userid=$DB_USER/$DB_PASSWD@$DB_SERVICE batch=yes source=$FILE stype=rdffile dtype=repfile overwrite=yes compile_all=yes 
done
...
reports_rdf_cnt=`ls -l $REPORTS_FOLDER/*.rdf | wc -l`
cd $REPORTS_FOLDER
ls -A1 *.rdf > reports_source.csv
reports_rep_cnt=`ls -l $REPORTS_OUTPUT/*.rep | wc -l`
cd $REPORTS_OUTPUT
ls -A1 *.rep > reports_compiled.csv

echo "Reports Source Count = " $reports_rdf_cnt
echo "Reports Compiled Count = " $reports_rep_cnt
