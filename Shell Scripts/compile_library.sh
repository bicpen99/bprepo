...
read -p "Enter the menu number of the application that you wish to compile (1-6): " app_name
  ...
  if [ $app_name == "1" ]; then
    ...
    cd $OUTPUT_PATH
    echo "Cleaning up old compiled Libraries"
    rm *.plx
...
for FILE in `ls *.pll`; do
  ...
  $ORACLE_HOME/bin/frmcmp $FILE $DB_USER/$DB_PASSWD@$DB_SERVICE module_type=library batch=yes output_file=$OUTPUT_PATH/$OUTPUT compile_all=special
done
