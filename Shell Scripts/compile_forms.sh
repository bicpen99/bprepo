...
read -p "Enter the menu number of the application that you wish to compile (1-6): " app_name
  if [ $app_name == "1" ]; then
    ...
    cd $OUTPUT_PATH
    echo "Cleaning up old compiled Forms, Menus and Libraries..."
    rm *.fmx
    rm *.mmx
    rm *.plx
    rm *.csv
...
for FILE in `ls *.pll`; do
  ...
  $ORACLE_HOME/bin/frmcmp_batch $FILE $DB_USER/$DB_PASSWD@$DB_SERVICE module_type=library batch=yes output_file=$OUTPUT_PATH/$OUTPUT compile_all=special
done
...
for FILE in `ls *.fmb`; do
  OUTPUT=${FILE/fmb/fmx}
  echo "Compiling Form $FILE..."
  $ORACLE_HOME/bin/frmcmp_batch $FILE $DB_USER/$DB_PASSWD@$DB_SERVICE module_type=form batch=yes output_file=$OUTPUT_PATH/$OUTPUT compile_all=special
done

comp_form_cnt=`ls -l $OUTPUT_PATH/*.fmx | wc -l`
cd $OUTPUT_PATH
ls -A1 *.fmx > compiled_forms.csv
forms_src_cnt=`ls -l $FORMS_PATH/*.fmb | wc -l`
cd $FORMS_PATH
ls -A1 *.fmb > source_code_forms.csv

echo "Forms Compiled Count = " $comp_form_cnt
echo "Forms Source Count = " $forms_src_cnt
