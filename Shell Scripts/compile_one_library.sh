...
echo "This script compiles one shared library for a specific application."
echo "	1. CASE		"
echo "	2. OGECRS	"
echo "	3. OGMENU	"
echo "	4. OGMOEA	"
...
read -p "Enter the menu number of the application that you wish to compile (1-7): " app_name
  ...
  export PATH=$FORMS_PATH:$PATH
  echo "FORMS_PATH = "$FORMS_PATH
  read -p "Enter a library to compile (enter the library filename, minus the extension): " answer
  echo "Compiling Library $answer..."
  $ORACLE_HOME/bin/frmcmp $FORMS_PATH/$answer.pll $DB_USER/$DB_PASSWD@$DB_SERVICE module_type=library batch=yes output_file=$OUTPUT_PATH/$answer.plx compile_all=special
